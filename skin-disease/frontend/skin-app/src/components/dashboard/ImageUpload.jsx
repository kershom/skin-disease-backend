...
const SYMPTOM_DEFINITIONS = [
  ...
  { key: 'bleeding', label: 'Bleeding', icon: Droplets },
  { key: 'burningSensation', label: 'Burning Sensation', icon: Flame },
  { key: 'scaling', label: 'Scaling', icon: Layers },
  { key: 'growthInSize', label: 'Growth in Size', icon: Maximize },
];
...
const ImageUpload = ({ images, setImages, selectedImageId, setSelectedImageId }) => {
  ...
  const handleSymptomChange = (imgId, symptomKey, value) => {
                  ...
                  <div className="bg-surface dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-border-subtle dark:border-slate-700 space-y-4">
                    <div className="flex items-center justify-between border-b border-border-subtle dark:border-slate-700 pb-3">
                      ...
                      <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-400 uppercase tracking-wide">
                        {activeImage.id.startsWith('webcam') ? (
                          <>
                            <Camera className="w-3 h-3 text-slate-400" />
                            <span>{t('dashboard.scan.webcamSnap', 'Webcam Snap')}</span>
                          </>
                        ) : (
                          <>
                            <Upload className="w-3 h-3 text-slate-400" />
                            <span>{t('dashboard.scan.uploadedImage', 'Uploaded Image')}</span>
                          </>
                        )}
                      </span>
                    </div>
                    ...
                    <div className="border-t border-border-subtle dark:border-slate-700 pt-5 mt-4 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="font-bold text-slate-800 dark:text-white text-xs uppercase tracking-wider">
                            {t('dashboard.scan.associatedSymptoms', 'Associated Symptoms')}
                          </h4>
                        </div>
                        <span className="text-[10px] font-bold bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 px-2.5 py-0.5 rounded-full border border-amber-100 dark:border-amber-900/40">
                          {t('dashboard.scan.developmentNote', 'Demo Mode')}
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {SYMPTOM_DEFINITIONS.map(sym => {
                          const isSelected = !!activeImage.symptoms?.[sym.key];
                          const Icon = sym.icon;
                          return (
                            <button
                              key={sym.key}
                              type="button"
                              onClick={() => handleSymptomChange(activeImage.id, sym.key, !isSelected)}
                            ...
                            >
                              ...
                              <span className="truncate">{t(`dashboard.scan.symptoms.${sym.key}`, sym.label)}</span>
                            </button>
                          );
                        })}
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
                          ...
                          <span>{t('dashboard.scan.durationLabel', 'Symptom Duration')}</span>
                        </div>
                        <div className="flex gap-2">
                          {DURATION_OPTIONS.map(opt => {
                            const isSelected = (activeImage.symptoms?.duration || '') === opt.value;
                            return (
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => handleSymptomChange(activeImage.id, 'duration', isSelected ? '' : opt.value)}
                      ...
                      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/60 rounded-xl p-3 flex gap-2.5 items-start">
                        <AlertCircle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                        <div className="text-xs text-amber-800 dark:text-amber-300 space-y-0.5">
                          <p className="font-semibold">{t('dashboard.scan.fusionTitle', 'Symptom-based decision fusion is under development')}</p>
                          <p className="text-[11px] text-amber-600 dark:text-amber-400 font-medium leading-relaxed">
                            {t('dashboard.scan.fusionDesc', 'Currently, prediction is based solely on the uploaded image. In a future update, these symptoms will be combined with image analysis to improve diagnostic accuracy.')}
                          </p>
                        </div>
                      </div>
                    </div>
