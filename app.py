...
# NOTE: this must be a real .keras file (a zip of metadata.json + config.json +
# model.weights.h5), not the three loose files. See skin_disease_efficientnet.keras
MODEL_PATH = os.path.join(os.path.dirname(__file__), "model", "skin_disease_efficientnet.keras")
 
# Matches the training folder order in dataset/processed (flow_from_directory /
# image_dataset_from_directory sort class folders alphabetically, and these
# 10 folders were already alphabetical: acne, athlete_foot, cellulitis, eczema,
# impetigo, ringworm, rosacea, shingles, urticaria_hives, vitiligo).
DISEASE_LABELS = [
    'Acne',              # index 0
    "Athlete's Foot",    # index 1
    'Cellulitis',        # index 2
    'Eczema',            # index 3
    'Impetigo',          # index 4
    'Ringworm',          # index 5
...
def preprocess_image(file_bytes):
    ...
    IMPORTANT: this EfficientNet model has its own Rescaling/Normalization
    layers baked in as the first layers of the network, so we must feed it
    RAW pixel values in [0, 255] as float32. Do NOT apply
    tf.keras.applications.*.preprocess_input here — that would double-process
    the image and wreck predictions.
    """
...
def generate_gradcam(img_array, class_index):
    ...
    try:
        ...
        # img_array is already raw pixels in [0, 255] (see preprocess_image),
        # so just clip/cast instead of multiplying by 255 again.
        original     = np.uint8(np.clip(img_array[0], 0, 255))
